...
def generate_default_values(DEFAULT_CONFIG_DIR):
    ...
    env_vars["LOG_DIR"] = os.path.join(env_vars["ROOT_DIR"], "log/")
    env_vars["ALLOWED_HOSTS"] = "*"  # This should be a space-separated list
    env_vars["LOG_LEVEL"] = "INFO"
    env_vars["DOMESTIC_DNS"] = [
        ("114.114.114.114", 53),
    ]
    env_vars["FOREIGN_DNS"] = [
        ("8.8.8.8", 53),
    ]
    # env_vars["ROOT_DIR"] =
...
def apply_env_from_list(env_var_list):
    ...
    check_and_create_dir(env_var_list["LOG_DIR"])
...
if __name__ == "__main__":
    ...
    if not isAlreadyExecuted:
        env_vars = main()
        ROOT_DIR        = env_vars["ROOT_DIR"]
        DATA_DIR        = env_vars["DATA_DIR"]
        CONFIG_DIR      = env_vars["CONFIG_DIR"]
        LOG_DIR         = env_vars["LOG_DIR"]
        ALLOWED_HOSTS   = env_vars["ALLOWED_HOSTS"]
        FOREIGN_DNS     = env_vars["FOREIGN_DNS"]
        DOMESTIC_DNS    = env_vars["DOMESTIC_DNS"]
        isAlreadyExecuted = True
    else:
        print("[WARNING] Settings: Settings already loaded.")
